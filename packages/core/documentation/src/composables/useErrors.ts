...
export default function useErrors() {
  ...
  const getMessageFromError = (error: any): string => {
    if (!error) {
      return i18n.t('documentation.errors.unexpected')
    }
    ...
    if (error?.response?.data) {
      ...
      if (error.response.data.details?.length) {
        return error.response.data.details.map((entry: Record<string, any>) => {
          let errors = i18n.t('documentation.errors.unexpected')
    ...
    return error.message || i18n.t('documentation.errors.unexpected')
  }
