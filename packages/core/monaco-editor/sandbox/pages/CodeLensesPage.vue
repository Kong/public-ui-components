  ...
  <div class="sandbox-model-context">
    ...
    <div class="controls-area">
      ...
      <KCard>
        ...
        <ul class="list">
          <li>Attaching code lenses to a specific JSON/YAML value at a key path.</li>
          <li>Decoding a X.509 certificate inline.</li>
          <li>Showing copy commands to all fields that contain string UUID values.</li>
          <li>Displaying formatted date/time to a specific JSON/YAML value at a key path.</li>
          <li>Listing formatted enum keys for a specific JSON/YAML value at a key path.</li>
          <li>Showing pretty name for a service id at a key path.</li>
          <li>Displaying code lenses for a specific JSON/YAML node type (e.g., boolean).</li>
          <li>...</li>
        </ul>
      </KCard>
    </div>
...
import 'reflect-metadata' // DO NOT MOVE
...
import { collectCodeLensProviders, MonacoEditor } from '../../src'
import {
  createJSONCopyUUIDCodeLensProvider,
  createJSONNodeCodeLensProvider,
  createJSONValueCodeLensProvider,
} from '../../src/languages/json/code-lenses'
import {
  createYAMLCopyUUIDCodeLensProvider,
  createYAMLNodeCodeLensProvider,
  createYAMLValueCodeLensProvider,
} from '../../src/languages/yaml/code-lenses'
import { trackDisposableForEditor } from '../../src/singletons/lifecycle'
import codeLensesJSON from '../assets/code-lenses.json'
import codeLensesYAML from '../assets/code-lenses.yaml?raw'
...
const unwatchEditor = watch(editor, (editor) => {
  ...
  trackDisposableForEditor(editor, languagesImport.registerCodeLensProvider('json', collectCodeLensProviders([
    createJSONCopyUUIDCodeLensProvider({
  ...
  trackDisposableForEditor(editor, languagesImport.registerCodeLensProvider('yaml', collectCodeLensProviders([
    createYAMLCopyUUIDCodeLensProvider({
      copyCommandId: COMMAND_ID_COPY,
      title: 'Copy UUID',
    }),

    createYAMLValueCodeLensProvider(['created_at'], (value, range) => {
      const formattedDateTime = new Date(Number(value)).toLocaleString('en', { dateStyle: 'full', timeStyle: 'full' })
    ...
    createYAMLValueCodeLensProvider(['certificate'], (value, range) => {
      let cert: x509.X509Certificate | undefined
      try {
        cert = new x509.X509Certificate(value)
      } catch (e) {
    ...
    createYAMLValueCodeLensProvider<{ id: string }>(['service'], (value, range) => {
      return {
        lenses: [
          {
            range,
    ...
    createYAMLValueCodeLensProvider<string[]>(['labels'], (value, range) => {
      return {
        lenses: value.reduce((lenses, label) => {
          let title: string
          if (label === 'debian12') {
    ...
    createYAMLNodeCodeLensProvider((node, stop, model) => {
      if (!isScalar(node) || typeof node.value !== 'boolean' || !node.range) return undefined
