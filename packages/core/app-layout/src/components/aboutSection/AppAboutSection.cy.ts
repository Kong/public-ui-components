...
describe('<AppAboutSection />', () => {
  ...
  it('should not render modified when it equals created', () => {
    ...
    cy.getTestId('about-section-timestamps-created').should('be.visible')
    cy.getTestId('about-section-timestamps-created').should('contain.text', created)
    cy.getTestId('about-section-timestamps-modified').should('not.be.visible')
  })
  ...
  it('should not render empty props/slots', () => {
    ...
    cy.get('.kong-ui-app-about-section').should('exist')
    cy.getTestId('about-section-title').should('not.be.visible')
    cy.getTestId('about-section-description').should('not.be.visible')
    cy.getTestId('about-section-timestamps-created').should('not.be.visible')
    cy.getTestId('about-section-timestamps-modified').should('not.be.visible')
    cy.getTestId('about-section-actions').should('not.be.visible')
    cy.getTestId('about-section-content').should('not.be.visible')
    cy.getTestId('about-divider-section').should('not.be.visible')
    cy.getTestId('about-divider-section-separator').should('not.be.visible')
  })
})
