...
7. **Fill the shared tooltip**: pass a `tooltipContent` function (`ChartTooltipContent`) to `ECharts` that maps the hovered item's ECharts params to `title` (the chart's `tooltipTitle` prop, from `BaseChartProps`), the context/metric subtitle and rows (`color`, `label`, formatted `value`, and `isExtra`/`isSegmentEmpty` where needed). Build it with the helpers in `src/utils/tooltip.ts`: `tooltipItems` (hovered items for any trigger), `categoryLabel` (index or name to label) and `tooltipRow` (color, label and formatted value). `HeatmapChart`'s `cellTooltipContent` is a short example. Keep customization declarative, like `@kong-ui-public/analytics-chart`: expose data props (e.g. `seriesName`, `valueFormatter`) rather than callbacks. Don't build a tooltip of your own: every chart uses the same one.
8. **Export the component** from `src/index.ts`.
9. **Add tests**:
...
10. **Add a sandbox page** under `sandbox/pages/`, and register it in `sandbox/index.ts` (route) and `sandbox/navigation.ts` (nav link).
11. **Add a doc** at `docs/x-chart.md` (props table, data shape, example), and link it from `README.md`.
