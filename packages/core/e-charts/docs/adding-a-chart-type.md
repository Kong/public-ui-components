...
7. **Fill the shared tooltip** by passing a `tooltipContent` function to `ECharts` that maps the hovered item's ECharts params to a title, an optional context/metric subtitle, and rows (`ChartTooltipContent`). Don't build a tooltip of your own: every chart uses the same one.
