  ...
  <div class="router-playground-wrapper">
    <div class="router-playground">
      <SupportText>
        ...
        >
          <template #link>
            <KExternalLink href="https://docs.konghq.com/gateway/latest/reference/expressions-language/language-references/">
...
const routeMatches = computed<{ [id: string]: boolean }>(() => requests.value.reduce((map, route) => {
  ...
  if (route.headers) {
    Object.entries(route.headers).forEach(([key, values]) => {
      // TODO: What if the header value is an array?
