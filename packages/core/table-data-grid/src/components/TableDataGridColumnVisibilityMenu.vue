<template>
  <KDropdown class="table-data-grid-column-visibility-menu">
    <KButton
      appearance="tertiary"
...
const filteredColumns = computed(() => {
  ...
  const normalizedSearch = debouncedSearchTerm.value.toLowerCase()
...
const toggleAll = () => {
  ...
  columnVisibility.value = {
    ...columnVisibility.value,
    ...Object.fromEntries(menuColumns.value.map(column => [column.key, !allVisible])),
  }
}
</script>

<style lang="scss" scoped>
$table-data-grid-column-menu-max-height: 250px;
$table-data-grid-column-label-max-width: 220px;

.table-data-grid-column-visibility-menu {
  ...
  .column-items {
    max-height: $table-data-grid-column-menu-max-height;
    overflow-y: auto;
  }
  ...
  .column-label {
    cursor: pointer;
    display: block;
    max-width: $table-data-grid-column-label-max-width;
    overflow: hidden;
