  ...
  <div class="sandbox">
    ...
    <main class="sandbox-main">
      <section class="table-section">
        <div
          id="table-data-grid-sandbox-outside-actions"
          class="sandbox-outside-actions-target"
        />
        <div class="sandbox-outside-controls">
          <div
            id="table-data-grid-sandbox-outside-search"
            class="sandbox-outside-control-target"
          />
          <div
            id="table-data-grid-sandbox-outside-filters"
            class="sandbox-outside-control-target"
          />
        </div>
        ...
        >
          ...
          >
            <Teleport to="#table-data-grid-sandbox-outside-actions">
...
import { createDefaultTableDataGridConfig } from '../src/utils/tableConfig'
...
const mode = ref<TableDataGridMode>('pagination')
const rowSelection = ref<TableDataGridRowSelectionMode>('multiple')
...
const lastRequest = ref<TableDataGridFetcherParams>()
const lastResponseCursor = ref<any>()
const fetchHistory = ref<Array<{
  fetchCount: number
  id: string
  request: TableDataGridFetcherParams
  responseCursor?: any
  title: string
  time: string
}>>([])
...
const rowSelectionOptions: Array<{ label: string, value: TableDataGridRowSelectionMode }> = [
...
const fetchHistoryModeLabels: Record<TableDataGridMode, string> = {
  infinite: 'Infinite',
  pagination: 'Pagination',
}
const generatedColumnKeyPrefix = 'demoColumn'
const generatedColumnWidth = 640
const isPinnedColumnValue = (value: string): value is Exclude<TableDataGridPinnedState, false> => (
...
const createDefaultColumnSettings = (): Array<TableDataGridHeader<MockRow>> => {
  const columns: Array<TableDataGridHeader<MockRow>> = [
...
const createDefaultTableConfig = (): TableDataGridConfig => {
...
const tableConfig = ref<TableDataGridConfig>(createDefaultTableConfig())
const columnSettings = ref<Array<TableDataGridHeader<MockRow>>>(createDefaultColumnSettings())
...
const headers = computed<Array<TableDataGridHeader<MockRow>>>(() => columnSettings.value.map(column => ({ ...column })))
...
const outsideFiltersTarget = computed(() => toolbarSlotMode.value === 'outside-filters'
  ? '#table-data-grid-sandbox-outside-filters'
  : undefined)
const outsideSearchTarget = computed(() => toolbarSlotMode.value === 'outside-search'
  ? '#table-data-grid-sandbox-outside-search'
  : undefined)

const createGeneratedColumn = (index: number): TableDataGridHeader<MockRow> => ({
  draggable: true,
  hideable: true,
...
const recordFetch = ({
  ...
  const positionLabels: Record<TableDataGridMode, string> = {
...
}: TableDataGridFetcherParams): Promise<TableDataGridFetcherResult<MockRow>> => {
  const request = {
    mode: fetchMode,
    page,
    pageSize,
...
const getRowAttrs: TableDataGridRowAttrs<MockRow> = row => ({
  'data-testid': `sandbox-row-${row.id}`,
})

const getCellAttrs: TableDataGridCellAttrs<MockRow> = ({ column, rowValue }) => ({
...
const handleTableConfigUpdate = (config: TableDataGridConfig) => {
