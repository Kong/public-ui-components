...
const config = mergeConfig(sharedViteConfig, defineConfig({
  ...
  optimizeDeps: {
    exclude: ['@kong/flame-chart-js'],
    include: ['color', 'events'],
  },
}))

// If we are trying to preview a build of the local `package/profiling/sandbox` directory,
