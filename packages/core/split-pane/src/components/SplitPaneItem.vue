...
import { ref, computed, inject, onMounted, onBeforeUnmount, watch, getCurrentInstance, nextTick, useTemplateRef } from 'vue'
...
const paneStyle = computed<CSSProperties>(() => {
  const horizontal = ctx?.horizontal?.value ?? false

  return {
    [horizontal ? 'height' : 'width']: `${pane.value?.size ?? 0}%`,
    flexShrink: 0,
    flexGrow: 0,
  }
})
...
const onMouseDown = (e: MouseEvent) => {
  ...
  const onMouseMove = (moveEvent: MouseEvent) => {
    const currentPos = ctx.horizontal.value ? moveEvent.clientY : moveEvent.clientX
...
onMounted(async () => {
  await nextTick()
  if (ctx?.registerPane && paneItemRef.value) {
    ctx.registerPane({
      id: uid,
      order: Number(order),
