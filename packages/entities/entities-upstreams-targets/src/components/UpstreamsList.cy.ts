...
describe('<UpstreamsList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-upstreams-list'

  const upstreamsWithManagedBy: FetcherRawResponse = {
    ...upstreams,
    data: upstreams.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(UpstreamsList, {
      props: {
        cacheIdentifier: `upstreams-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
