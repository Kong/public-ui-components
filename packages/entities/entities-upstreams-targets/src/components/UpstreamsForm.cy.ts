...
describe('<UpstreamsForm/>', { viewportHeight: 700, viewportWidth: 700 }, () => {
  describe('Konnect', () => {
    ...
    it('Should set correct values for health checks when turned off, then turned on the active again', () => {
      interceptFetchServices()
      interceptFetchCertificates()
      interceptGetUpstream(200, upstreamsResponseFull)
      interceptValidate()
      ...
      cy.wait('@updateUpstream').then((interception) => {
        const { body: { healthchecks } } = interception.request
        expect(healthchecks).to.deep.equal(JSON.parse('{ "threshold": 2, "active": { "type": "https", "healthy": { "interval": 5, "successes": 5, "http_statuses": [ 200, 302 ] }, "unhealthy": { "interval": 5, "timeouts": 5, "tcp_failures": 5, "http_failures": 5, "http_statuses": [ 429, 404, 500, 501, 502, 503, 504, 505 ] }, "timeout": 1, "concurrency": 10, "http_path": "/", "https_sni": null, "https_verify_certificate": true } }'))
      })
    })
