<template>
  <div class="kong-ui-entities-certificates-list">
    ...
    >
      ...
      >
        <EntityEmptyState
          :action-button-text="t('certificates.list.empty_state_v2.create')"
          appearance="secondary"
          :can-create="() => canCreate()"
          data-testid="certificates-entity-empty-state"
          :description="t('certificates.list.empty_state_v2.description')"
      ...
      <template #subject="{ row }">
        <b v-if="config.app === AppType.Konnect">{{ row?.metadata?.subject ? row.metadata.subject : '-' }}</b>
        <b v-else>{{ getCertificateData(row).schemaSubject || '-' }}</b>
      </template>
      <template #expiry="{ row }">
        <span v-if="config.app === AppType.Konnect">
      ...
      <template #san="{ row }">
        <span v-if="config.app === AppType.Konnect">{{ row?.metadata?.dns_names ? `dns=${row.metadata.dns_names.join(', ')}` : '-' }}</span>
        <span v-else>
