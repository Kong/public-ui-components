...
describe('<CACertificateList />', () => {
  ...
  describe('Konnect', () => {
    ...
    it('should show empty state and create ca certificate cta', () => {
      ...
      cy.wait('@getCaCertificate')
      cy.get('.kong-ui-entities-ca-certificates-list').should('be.visible')
      cy.getTestId('ca-certificates-entity-empty-state').should('be.visible')
...
describe('<CACertificateList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-ca-certificates-list'

  const caCertificateWithManagedBy: FetcherRawResponse = {
    ...caCertificate,
    data: caCertificate.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(CACertificateList, {
      props: {
        cacheIdentifier: `ca-certificate-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
  ...
  beforeEach(() => {
    ...
    cy.intercept(
      ...
      {
        statusCode: 200,
        body: caCertificateWithManagedBy,
      },
    )
  })
  ...
  it('omits the managed_by column entirely while the flag is off', () => {
    ...
    cy.get(`${managedByColumnClass} thead th`).should('contain.text', 'Issuer')
