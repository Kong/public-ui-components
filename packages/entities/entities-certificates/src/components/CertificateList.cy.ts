...
describe('<CertificateList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-certificates-list'

  const certificateWithManagedBy: FetcherRawResponse = {
    ...certificate,
    data: certificate.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(CertificateList, {
      props: {
        cacheIdentifier: `certificate-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
  ...
  it('omits the managed_by column entirely while the flag is off', () => {
    ...
    cy.get(`${managedByColumnClass} thead th`).should('contain.text', 'Cert')
