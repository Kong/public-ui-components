...
describe('<SniList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-snis-list'

  const snisWithManagedBy: FetcherRawResponse = {
    ...snis,
    data: snis.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(SniList, {
      props: {
        cacheIdentifier: `sni-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
