...
describe('<RouteList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-routes-list'

  const routesWithManagedBy: FetcherRawResponse = {
    ...routes,
    data: routes.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(RouteList, {
      props: {
        cacheIdentifier: `route-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
