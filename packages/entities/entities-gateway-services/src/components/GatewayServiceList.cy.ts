...
describe('<GatewayServiceList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-gateway-services-list'

  const baseConfigKonnect: KonnectGatewayServiceListConfig = {
    app: 'konnect',
    controlPlaneId: '1234-abcd-ilove-cats',
    apiBaseUrl: '/us/kong-api',
    createRoute: 'create-gateway-service',
    getViewRoute: () => 'view-gateway-service',
    getEditRoute: () => 'edit-gateway-service',
  }

  const gatewayServicesWithManagedBy: FetcherRawResponse = {
    ...gatewayServices,
    data: gatewayServices.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(GatewayServiceList, {
      props: {
        cacheIdentifier: `gateway-service-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
  ...
  beforeEach(() => {
    ...
    cy.intercept(
      ...
      {
        statusCode: 200,
        body: gatewayServicesWithManagedBy,
      },
    )
  })
