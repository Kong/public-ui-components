...
describe('<KeyList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-keys-list'

  const keysWithManagedBy: FetcherRawResponse = {
    ...keys,
    data: keys.data.map(row => ({
      ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(KeyList, {
      props: {
        cacheIdentifier: `key-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
