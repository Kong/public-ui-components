...
describe('<KeyForm />', () => {
  describe('Kong Manager', () => {
    ...
    it('should show create form', () => {
      ...
      // button state
      cy.getTestId('key-form-cancel').should('be.visible')
      cy.getTestId('key-form-submit').should('be.visible')
      cy.getTestId('key-form-cancel').should('be.enabled')
      cy.getTestId('key-form-submit').should('be.disabled')
      // form fields
    ...
    it('should correctly handle button state - create', () => {
      ...
      cy.getTestId('key-form-submit').should('be.enabled')
    ...
    it('should pick correct url while creating global key', () => {
      ...
      cy.get(`[data-testid="select-item-${keySets.data[0].id}"] button`).click()
      cy.getTestId('key-form-jwk').type(jwkString, { delay: 0 })
      cy.getTestId('key-form-submit').click()
      cy.wait('@createKey')
    })
    ...
    it('should handle error state - failed to load Key', () => {
      ...
      cy.getTestId('key-form-cancel').should('not.exist')
      cy.getTestId('key-form-submit').should('not.exist')
