...
describe('<ConsumerList /> - managed_by column', () => {
  const managedByColumnClass = '.kong-ui-entities-consumers-list'
  ...
  const consumersWithManagedBy = consumers5.map(row => ({
    ...row,
  ...
  const mountList = (options: { managedByEnabled?: boolean } = {}) => {
    cy.mount(ConsumerList, {
      props: {
        cacheIdentifier: `consumer-list-managed-by-${uuidv4()}`,
        config: baseConfigKonnect,
        canCreate: () => false,
        canEdit: () => false,
        canDelete: () => false,
  ...
  beforeEach(() => {
    ...
    cy.intercept(
      ...
      },
      {
        statusCode: 200,
        body: {
          data: consumersWithManagedBy,
          total: consumersWithManagedBy.length,
        },
      },
    )
  })
