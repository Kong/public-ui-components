...
import { DATA_INJECTION_KEY, FIELD_RENDERER_MATCHERS_MAP, FIELD_RENDERER_SLOTS, SCHEMA_INJECTION_KEY, useSchemaHelpers, FIELD_RENDERERS } from './composables'
...
const { tag = 'form', schema, data, prepareFormData } = defineProps<{
  tag?: string
  schema: FormSchema
  data?: T
...
const formData = reactive<T>(
  prepareFormData
    ? prepareFormData(data || schemaHelpers.getDefault())
