...
export function useFieldLabel(
  ...
  return computed(() => {
    ...
    const parentIsArrayOrMap = parentSchema?.type === 'array' || parentSchema?.type === 'map'

    const res = parentIsArrayOrMap
      ? '' // hide the label when it is a direct child of Array or Map
