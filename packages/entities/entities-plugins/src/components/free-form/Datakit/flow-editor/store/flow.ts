...
const [provideFlowStore, useOptionalFlowStore] = createInjectionState(
  function(options: UseFlowOptions) {
    ...
    async function autoLayout(commitNow = true) {
      ...
      nodes.value.forEach((node) => nodeMap.set(node.id, node.data!))

      const deepFirstGroups = state.value.groups.toSorted((a, b) => getNodeDepth(b.ownerId) - getNodeDepth(a.ownerId))
      for (const group of deepFirstGroups) {
        doAutoLayout(
