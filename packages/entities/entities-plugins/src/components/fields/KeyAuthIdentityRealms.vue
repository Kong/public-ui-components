...
const onRealmsUpdate = (value: string[]) => {
  const selectedCurrentCP = value.includes('current-cp')
    ? [{ scope: 'cp', id: null } as const] : []

  const selectedKonnectRealms = value
    .filter((realmId) => realmId !== 'current-cp')
    .map((realmId) => ({ scope: 'realm', id: realmId } as const))

  fieldValue.value = [...selectedCurrentCP, ...selectedKonnectRealms]
}
