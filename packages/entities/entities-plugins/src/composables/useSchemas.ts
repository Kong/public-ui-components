...
export const useSchemas = (options?: UseSchemasOptions) => {
  ...
  const evaluateFeatureFlag = useEvaluateFeatureFlag()
  ...
  const parseSchema = (currentSchema: Record<string, any>, backendSchema?: Record<string, any>, frontendSchema?: Record<string, any>) => {
    ...
    if (getSharedFormName(pluginName) || getFreeFormName(pluginName, evaluateFeatureFlag) || metadata?.useLegacyForm || options?.credential) {
      /**
