...
export const useSchemas = (entityId?: string, options?: UseSchemasOptions) => {
  ...
  const parseSchema = (currentSchema: Record<string, any>, backendSchema?: Record<string, any>, frontendSchema?: Record<string, any>) => {
    ...
    if (!getSharedFormName(pluginName) && options?.groupFields) {
      ...
      if (metadata.fieldChecks) {
        for (const fieldSets of [
          metadata.fieldChecks.atLeastOneOf,
          metadata.fieldChecks.onlyOneOf,
          metadata.fieldChecks.mutuallyRequired,
          metadata.fieldChecks.exactOneOf,
        ]) {
          if (fieldSets) {
            for (const fields of fieldSets) {
              for (const field of fields) {
                // We flatten the schema with hyphen notation
                checkedFields[field.replace(/-/g, '_').replace(/\./g, '-')] = false // not visited yet
              }
            }
          }
        }

        if (metadata.fieldChecks.exactOneOfMutuallyRequired) {
          for (const combinations of metadata.fieldChecks.exactOneOfMutuallyRequired) {
      ...
      for (const field of formSchema.fields!) {
        ...
        // A field is considered required if any of the following is true:
        // - It has a `required` property and it's set to true
        // - It's part of a set of fields where at least one of them is required
        if (field.required || checkedFields[field.model] !== undefined) {
          if (checkedFields[field.model] === false) {
            checkedFields[field.model] = true // visited
          }
      ...
      const unknownCheckedFields = Object.entries(checkedFields)
        .filter(([, visited]) => !visited)
        .map(([field]) => field.replace(/-/g, '.').replace(/_/g, '-'))
      if (unknownCheckedFields.length > 0) {
        console.warn(`Unknown checked fields for plugin ${pluginName}: ${unknownCheckedFields.join(', ')}`)
      }
      ...
      if (basicFields.length > 0 || advancedFields.length > 0) {
        fieldGroups.push({
          fields: basicFields.sort(sortFieldByOrderAndModel),
          collapsible: {
