...
describe('<AnalyticsDatatable />', () => {
  const mountTable = ({
    ...
    onState?: ((payload: AnalyticsDatatableStatePayload) => void) | undefined
  ...
  const mountSyncedTable = ({
    ...
    fetcher?: AnalyticsDatatableFetcher<TestRow>
    headers?: Array<AnalyticsDatatableHeader<TestRow>>
    defaultRowSelection?: AnalyticsDatatableRowSelectionMode
    defaultMode?: AnalyticsDatatableMode
    onGridReady?: ((api: GridApi<TestRow>) => void) | undefined
    onUpdateTableConfig?: ((nextConfig: AnalyticsDatatableConfig) => void) | undefined
  } = {}) => {
    const hostConfig = ref(config)
    const syncedHeaders = ref(tableHeaders)
    const rowSelection = ref<AnalyticsDatatableRowSelectionMode>(defaultRowSelection)
    const mode = ref<AnalyticsDatatableMode>(defaultMode)
  ...
  const expectNoHorizontalOverflow = () => {
    cy.get('.analytics-datatable-grid .ag-body-horizontal-scroll-viewport').then(($viewport) => {
      expect($viewport[0].scrollWidth).to.be.lessThan($viewport[0].clientWidth + 20)
    })
  }

  const expectHorizontalOverflow = () => {
    cy.get('.analytics-datatable-grid .ag-center-cols-viewport').then(($viewport) => {
  ...
  const hideColumnInHostConfigBeforeSizing = (
    hostConfig: Ref<AnalyticsDatatableConfig>,
    columnKey: string,
  ) => {
    cy.then(() => {
      hostConfig.value = {
    ...
    cy.getTestId(`analytics-datatable-header-${columnKey}`).should('not.exist')
  }
  ...
  describe('rendering and states', () => {
    ...
    it('provides rowValue to cell slots', () => {
      mountTable({
        slots: {
          latency: ({ rowValue }: AnalyticsDatatableCellSlotProps<TestRow>) => h('span', {
    ...
    it('renders localized labels for table chrome', () => {
      ...
      cy.getTestId('analytics-datatable-bulk-actions-trigger').should('contain.text', '(0) Bulk actions')
    ...
    it('renders custom empty and error states', () => {
      ...
      cy.getTestId('analytics-datatable-empty').should('not.exist')
      ...
      cy.getTestId('analytics-datatable-error').should('not.exist')
    })
    ...
    it('renders error state and emits state for a failed first infinite block', () => {
      ...
      cy.getTestId('analytics-datatable-error').should('contain.text', 'Unable to load data')
      cy.get('@state').should('have.been.calledWithMatch', {
        state: 'error',
        hasData: false,
      })
  ...
  describe('column sizing and overflow', () => {
    ...
    it('allows horizontal overflow when restoring a hidden column in a pinned layout cannot fit', () => {
      let gridApi: GridApi<TestRow> | undefined
      const headersWithPinnedColumn: Array<AnalyticsDatatableHeader<TestRow>> = [
    ...
    it('keeps dynamically added wide configured columns visible after layout state exists', () => {
      let gridApi: GridApi<TestRow> | undefined
      const wideHeader: AnalyticsDatatableHeader<TestRow> = {
        key: 'details',
        label: 'Details',
        width: 900,
      }
      ...
      cy.getTestId('analytics-datatable-header-details').should('exist')
      waitForColumnSizing()
      cy.then(() => {
    ...
    it('allows added configured-width columns to overflow after a pending visibility refit', () => {
      let gridApi: GridApi<TestRow> | undefined
      const addedHeaders: Array<AnalyticsDatatableHeader<TestRow>> = Array.from({ length: 4 }, (_, index) => ({
        key: `extra-${index + 1}`,
        label: `Extra ${index + 1}`,
        width: 640,
      }))
  ...
  describe('toolbar and outside actions', () => {
    it('renders the default toolbar and selected row actions trigger', () => {
      ...
      cy.getTestId('analytics-datatable-search').should('not.exist')
    ...
    it('renders search in the toolbar when search is enabled', () => {
      ...
      cy.getTestId('analytics-datatable-search').should('be.visible')
      cy.getTestId('outside-search-target').findTestId('analytics-datatable-search').should('not.exist')
    ...
    it('keeps toolbar search available when search results are empty', () => {
      ...
      cy.contains('Gateway service').should('be.visible')
      cy.getTestId('analytics-datatable-search').type('no matching service')
      cy.getTestId('analytics-datatable-empty').should('be.visible')
      cy.getTestId('analytics-datatable-search')
        .should('be.visible')
    ...
    it('teleports built-in search when outsideSearch is provided', () => {
      ...
      cy.get('.datatable-toolbar').find('[data-testid="analytics-datatable-search"]').should('not.exist')
      cy.getTestId('outside-search-target').findTestId('analytics-datatable-search').should('be.visible')
      cy.clock()
      cy.getTestId('outside-search-target').findTestId('analytics-datatable-search').type('Service')
      cy.tick(350)
      cy.wrap(fetcher).should('have.been.calledWithMatch', {
        mode: 'pagination',
        page: 1,
    ...
    it('lets outside-actions update search and refresh the table', () => {
      const fetcher = createPaginatedFetcher()
      let updateOutsideSearch: AnalyticsDatatableToolbarSlotProps<TestRow>['updateSearch'] | undefined
      ...
      mountTable({
        ...
        slots: {
          'outside-actions': ({
            search,
            updateSearch,
          }: AnalyticsDatatableToolbarSlotProps<TestRow>) => {
    ...
    it('renders outside-actions while the toolbar is hidden', () => {
      mountTable({
        hideToolbar: true,
        slots: {
          'outside-actions': ({ selectedRows }: AnalyticsDatatableToolbarSlotProps<TestRow>) => h('span', {
    ...
    const outsideActionsMountedStateCases = [
      {
        ...
        expectedStateTestId: 'analytics-datatable-loading',
        label: 'loading',
      },
      {
        actionTestId: 'outside-error-action',
        createProps: () => ({ error: true }),
        expectedStateTestId: 'analytics-datatable-error',
        label: 'error',
      },
      {
        actionTestId: 'outside-empty-action',
        createProps: () => ({ fetcher: cy.stub().resolves({ data: [], total: 0 }) }),
        expectedStateTestId: 'analytics-datatable-empty',
        label: 'empty',
      },
    ]
    ...
    it('lets outside-actions update filters and refresh the table', () => {
      ...
      let updateOutsideFilterSelection: AnalyticsDatatableToolbarSlotProps<TestRow>['updateFilterSelection'] | undefined
  ...
  describe('filters and headers', () => {
    ...
    it('keeps outside search and filters mounted across non-grid states', () => {
      ...
      outsideControlStateCases.forEach(({ createProps, expectedStateTestId }) => {
        ...
        cy.getTestId(expectedStateTestId).should('be.visible')
        cy.getTestId('outside-search-target').findTestId('analytics-datatable-search').should('exist')
  ...
  describe('pagination and page size', () => {
    ...
    it('can hide pagination controls', () => {
      ...
      cy.contains('Gateway service').should('be.visible')
      cy.getTestId('analytics-datatable-pagination').should('not.exist')
    })
  ...
  describe('sorting and infinite mode', () => {
    ...
    it('rebuilds the infinite datasource with search when search changes', () => {
      ...
      cy.getTestId('outside-search-target').findTestId('analytics-datatable-search').type('Infinite')
      cy.wrap(fetcher).should('have.been.calledWithMatch', {
        mode: 'infinite',
        startRow: 0,
        cursor: undefined,
    ...
    it('keeps the infinite grid mounted while sorting refetches the first block', () => {
      ...
      cy.getTestId('analytics-datatable-header-latency').click()
      cy.getTestId('analytics-datatable-empty').should('not.exist')
      cy.contains('Infinite service 1').should('be.visible')
      cy.wrap(fetcher).should('have.been.calledWithMatch', {
        mode: 'infinite',
