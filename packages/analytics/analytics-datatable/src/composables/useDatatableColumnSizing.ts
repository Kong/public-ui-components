...
export const useDatatableColumnSizing = <Row extends Record<string, any>>({
  ...
  updateTableConfig: (config: AnalyticsDatatableConfig) => void
}) => {
  ...
  const getColumnFitMinWidth = (header: AnalyticsDatatableHeader<Row>) => (
  ...
  const isHeaderVisible = (header: AnalyticsDatatableHeader<Row>) => (
  ...
  const getConfigForChangeDetection = (
    gridConfig: AnalyticsDatatableConfig,
