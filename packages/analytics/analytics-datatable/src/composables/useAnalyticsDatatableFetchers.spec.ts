...
import type { AnalyticsDatatableFetcher } from '../types'
...
describe('useAnalyticsDatatableFetchers', () => {
  ...
  it('infers unknown-total next-page state from returned page size when hasMore is omitted', async () => {
    ...
    const fetcher = ref<AnalyticsDatatableFetcher<TestRow>>(fullPageFetcher)
    const {
      fetchPage,
      hasNextPageWhenTotalUnknown,
    } = useAnalyticsDatatableFetchers<TestRow>({
