...
describe('useDatatableSelection', () => {
  ...
  const createSelection = ({
    agGridOptions = ref<AnalyticsDatatableGridOptions<TestRow>>({}),
    gridApi = ref<GridApi<TestRow>>(),
    rowSelection = ref<AnalyticsDatatableRowSelectionMode>('multiple'),
  } = {}) => {
  ...
  it('resolves row selection mode and selection column configuration', () => {
    const rowSelection = ref<AnalyticsDatatableRowSelectionMode>('none')
    const agGridOptions = ref<AnalyticsDatatableGridOptions<TestRow>>({
      selectionColumnDef: {
        width: 72,
      },
    })
