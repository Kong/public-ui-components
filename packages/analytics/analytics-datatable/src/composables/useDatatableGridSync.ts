...
const createLayoutSnapshot = (config: AnalyticsDatatableConfig): AnalyticsDatatableConfig => ({
...
export const useDatatableGridSync = <Row extends Record<string, any>>({
  ...
  emitSort: (sort: AnalyticsDatatableSort) => void
  getGridConfig: (api: GridApi<Row>) => AnalyticsDatatableConfig
  gridApi: Ref<GridApi<Row> | undefined>
  hasFetched: Ref<boolean>
  patchTableConfig: (config: Partial<AnalyticsDatatableConfig>) => void
  refresh: (params?: Partial<AnalyticsDatatableSort & { pageSize: number }>) => void
  resolvedSort: Readonly<Ref<Pick<AnalyticsDatatableSort, 'sortColumnKey' | 'sortColumnOrder'>>>
...
const hasSelectionColumn = (selectionMode: AnalyticsDatatableRowSelectionMode) => selectionMode === 'multiple'
