# Analytics Datatable Architecture

`@kong-ui-public/analytics-datatable` is a Vue wrapper around AG Grid for analytics-style tables. Consumers provide typed headers, a fetcher, optional controlled table configuration, filter state, and slots. The package translates that public contract into AG Grid state, fetches rows, renders table chrome, and emits user-driven changes back to the host.
...
flowchart LR
  ...
  Component["AnalyticsDatatable.vue<br/>orchestration and template"]
  Config["useAnalyticsDatatableConfig<br/>controlled config state"]
  Fetchers["useAnalyticsDatatableFetchers<br/>pagination and infinite requests"]
...
| Area | File | Owns |
| --- | --- | --- |
| Wrapper orchestration | [`src/components/AnalyticsDatatable.vue`](../src/components/AnalyticsDatatable.vue) | Template, AG Grid wiring, toolbar state, event handlers, and composable coordination |
| Public contract | [`src/types/index.ts`](../src/types/index.ts) | Header, config, fetcher, row key, selection, slot, and attribute types |
| Config state | [`src/composables/useAnalyticsDatatableConfig.ts`](../src/composables/useAnalyticsDatatableConfig.ts) | Internal active config, resolved config, applying config to AG Grid, emitting config updates |
...
| Fetch lifecycle | [`src/composables/useAnalyticsDatatableFetchers.ts`](../src/composables/useAnalyticsDatatableFetchers.ts) | Pagination requests, infinite datasource creation, cursor tracking, stale request guards |
...
| Cell renderer | [`src/components/AnalyticsDatatableCellRenderer.vue`](../src/components/AnalyticsDatatableCellRenderer.vue) | Column-key slots, cell attrs, selected slot state, AG Grid renderer refresh |
| Header renderer | [`src/components/AnalyticsDatatableHeaderRenderer.vue`](../src/components/AnalyticsDatatableHeaderRenderer.vue) | Sort button, sort icon state, tooltip rendering |
| Column visibility menu | [`src/components/AnalyticsColumnVisibilityMenu.vue`](../src/components/AnalyticsColumnVisibilityMenu.vue) | Hideable-column menu, search, toggle-all visibility updates |
...
sequenceDiagram
  participant Host
  participant Table as AnalyticsDatatable.vue
  participant Config as useAnalyticsDatatableConfig
...
flowchart LR
  ...
  Grid["AG Grid"]
  HeaderRenderer["AnalyticsDatatableHeaderRenderer"]
...
| --- | --- | --- |
| Public props, emits, or exported types | `src/types/index.ts`, `AnalyticsDatatable.vue`, README | Treat as public API unless intentionally breaking |
| Config normalization or equality | `src/utils/tableConfig.ts` | Preserve explicit `pinnedColumns[key] = false` semantics |
| Sort/page-size-driven refreshes | `AnalyticsDatatable.vue` and `useAnalyticsDatatableConfig` | Sort and page size are config changes that trigger fetch refresh |
| Pagination or infinite fetch behavior | `useAnalyticsDatatableFetchers` | Preserve stale request guards |
...
| Filter ownership | `AnalyticsDatatable.vue` | Built-in filters update `filterSelection`; custom filter slots are host-owned |
