...
const zTimeRange = z.union([zExploreV4RelativeTime, zExploreV4AbsoluteTime]).optional().default({
...
const zFilters = <T extends readonly string[]>(filterableDimensions: T) => {
  ...
  return z.array(z.union([zInFilter, zEmptyFilter])).optional()
}

// Query schemas (per datasource)
export const zApiUsageQuery = zBaseQuery.extend({
  datasource: z.literal('api_usage'),
  metrics: zMetrics(exploreAggregations),
  dimensions: zDimensions(queryableExploreDimensions),
  filters: zFilters(filterableExploreDimensions),
}).strict()
export const zBasicQuery = zBaseQuery.extend({
  datasource: z.literal('basic'),
  metrics: zMetrics(basicExploreAggregations),
  dimensions: zDimensions(queryableBasicExploreDimensions),
  filters: zFilters(filterableBasicExploreDimensions),
}).strict()
export const zLlmUsageQuery = zBaseQuery.extend({
  datasource: z.literal('llm_usage'),
  metrics: zMetrics(aiExploreAggregations),
  dimensions: zDimensions(queryableAiExploreDimensions),
  filters: zFilters(filterableAiExploreDimensions),
}).strict()

export const zValidDashboardQuery = z.union([zApiUsageQuery, zBasicQuery, zLlmUsageQuery])
